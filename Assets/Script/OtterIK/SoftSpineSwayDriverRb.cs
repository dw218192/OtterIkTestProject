...
public class SoftSpineSwayDriverRb : MonoBehaviour
{
    [Serializable]
    public class Bone
    {
    ...
    // body yaw angular velocity estimator (deg/s)
    private Vector3 _prevForwardFlat;
    private bool _hasPrevForward;
    ...
    private void LateUpdate()
    {
        ...
        float omega = EstimateOmegaDeg(dt);
        ...
        float speed01 = 0f;
        {
            float denom = Mathf.Max(0.001f, movement.GetSprintSpeed());
            speed01 = Mathf.Clamp01(movement.GetSpeed() / denom);
        }
    ...
    private float EstimateOmegaDeg(float dt)
    {
        // Signed yaw rate around world up, derived from change in character forward.
        // Works for RB and Transform rotation alike (with X/Z rotations frozen in MovementControllerRB).
        Vector3 worldUp = GetWorldUp();

        Vector3 f = (characterRoot != null ? characterRoot.forward : transform.forward);
        f.y = 0f;
        if (f.sqrMagnitude < 1e-6f) f = Vector3.forward;
        f.Normalize();

        if (!_hasPrevForward || dt <= 1e-6f)
        {
            _prevForwardFlat = f;
            _hasPrevForward = true;
            return 0f;
        }

        // Note: using planar vectors, so worldUp is effectively (0,1,0), but we keep it generic.
        float signedDelta = Vector3.SignedAngle(_prevForwardFlat, f, worldUp);
        _prevForwardFlat = f;

        return signedDelta / dt;
    }
