...
public class FrontPaddleController_neo : MonoBehaviour
{
    ...
    [Header("Optional Movement Controller (for carrot direction)")]
    ...
    private void Update()
    {
        ...
        // --- compute omega from carrot heading error (stable even at idle) ---
        float angleError = 0f;

        if (movement != null)
        {
            Vector3 carrotW = movement.GetCarrotWorld();
            Vector3 toCarrot = carrotW - transform.position;
            toCarrot.y = 0f;
            ...
            if (toCarrot.sqrMagnitude > 1e-6f)
            {
                ...
                angleError = Vector3.SignedAngle(fwd, toCarrot, Vector3.up); // deg
            }
        }
        ...
        _omegaDeg = Mathf.Lerp(_omegaDeg, desiredOmega, aOmega);
        ...
        bool want = canPaddleNow && (omegaAbs >= omegaStartThreshold);
